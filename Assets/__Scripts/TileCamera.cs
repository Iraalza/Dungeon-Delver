...
public class TileCamera : MonoBehaviour
{
    ...
    public void LoadMap()
    {
        GameObject go = new GameObject("TILE_ANCHOR"); //создаем эту штуку, она играет роль родителя всех плиток Tile
        TILE_ANCHOR = go.transform;

        SPRITES = Resources.LoadAll<Sprite>(mapTiles.name);//загружаю спрайты

        //читаю информацию для карты
        ...
        //Сохранение информации для карты в двумерной массив для ускорения доступа
        MAP = new int[W, H];
        for (int j=0; j<H; j++)
        {
    ...
    /// <summary>
    /// Генерирует плитки для всей карты
    /// </summary>
    void ShowMap()
    {
        TILES = new Tile[W, H];
